...
const CLI_PATH = "./dist/chcli.js";

beforeAll(async () => {
  await $`bun run build`.quiet();
  // Fail fast if ClickHouse is not reachable
  await $`bun ${CLI_PATH} -q "SELECT 1"`.quiet();
});

describe("e2e", () => {
  test("SELECT 1 returns 1", async () => {
    const result = await $`bun ${CLI_PATH} -q "SELECT 1 AS val" -F csv`.text();
  ...
  test("inline query with json format", async () => {
    const result = await $`bun ${CLI_PATH} -q "SELECT 42 AS answer" -F json`.text();
  ...
  test("pipe query via stdin", async () => {
    const result =
      await $`echo "SELECT 'hello' AS greeting" | bun ${CLI_PATH} -F csv`.text();
    const lines = result.trim().split("\n");
  ...
  test("query from file", async () => {
    ...
    const result = await $`bun ${CLI_PATH} -f ${tmpFile} -F csv`.text();
    const lines = result.trim().split("\n");
    expect(lines[1]).toBe("99");
  });

  test("--time flag prints elapsed time", async () => {
    const result = await $`bun ${CLI_PATH} -q "SELECT 1" -t -F csv`.nothrow();
  ...
  test("tsv format works", async () => {
    const result =
      await $`bun ${CLI_PATH} -q "SELECT 1 AS a, 2 AS b" -F tsv`.text();
    const lines = result.trim().split("\n");
  ...
  test("SHOW DATABASES returns results", async () => {
    const result = await $`bun ${CLI_PATH} -q "SHOW DATABASES" -F csv`.text();
