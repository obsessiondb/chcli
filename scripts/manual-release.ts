 ...
 *   4. Runs quality gates (typecheck, test)
 *   5. Publishes the package via `bun publish`
...
export async function main(): Promise<void> {
	...
	// 4. Quality gates (typecheck, test)
	runQualityGates()
...
function publishPackage(otp: string): void {
	const pkgJsonPath = resolve('package.json')
	const pkg = JSON.parse(readFileSync(pkgJsonPath, 'utf8')) as PackageJson

	if (!pkg.name || !pkg.version) {
		fail('package.json is missing name or version.')
	}

	if (isVersionPublished(pkg.name, pkg.version)) {
		fail(`${pkg.name}@${pkg.version} is already published on npm. Nothing to do.`)
	}
